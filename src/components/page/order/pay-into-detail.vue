<template>
	<div class="payDetail text-l" id="payDetail">
		...
		<div class="pay-cont">
			<div class="detail-item"><label>订单号：</label><span>{{ detailInfo.recordNo || '--' }}</span></div>
			...
			<div class="bd-line"></div>
			<div class="detail-item"><label>支付金额：</label><span>{{ detailInfo.orderAmount || '00' }}</span></div>
	...
	export default {
		...
		methods: {
			getOrderDetail(orderId) {
				axios.post('http://192.168.0.254:8093/record/getOneRecord', {
					data: {
						id: orderId,
					}
				}).then((response) => {
