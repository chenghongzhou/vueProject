<template>
	<section class="pay-out" v-cloak>
			...
			<el-row class="uinn-20">
				<el-col :span="24">
					...
					<el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline text-l">
						...
						<el-form-item label="支付订单号" prop="withdrawNo">
							<el-input class="input" v-model="formInline.withdrawNo"></el-input>
						</el-form-item>
					...
					<el-table :data="tableData" border style="width: 100%">
						...
						<el-table-column prop="withdrawNo" label="支付订单号">
						</el-table-column>
		...
		<div class="payDetail text-l" id="payDetail" v-if="showDetail">
			...
			<div class="pay-cont">
				<div class="detail-item"><label>支付订单号：</label><span>{{ detailInfo.withdrawNo || '--' }}</span></div>
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		methods: {
			...
			getErrorData() {
				const url = common.apihost + 'sysDictionary/list-children';
				$http({
			...
			getLoadData(pageNum, pageSize) {
				this.isLoad = true;
				$http({
					url: common.apihost + 'withdraw/getWithdrawList',
					data: {
						data: {
							page: pageNum,
							pagesize: pageSize,
							withdrawNo: this.formInline.withdrawNo,
			...
			download() {
				$http({
					url: common.apihost + 'recordExcle/poi',
					method: 'post'
				}).then((response) => {});
			},
			...
			selectDetail(id) {
				...
				$http({
					url: common.apihost + 'withdraw/getOnewithdraw',
					data: {
						data: {
							id: id
						}
