			...
			<el-col :span="24" class="uinn-20">
				...
				<el-table :data="tableData" border style="width: 100%">
					...
					<!-- <el-table-column label="累计支入金额">
						<template scope="scope">
							{{ '￥'+Number(scope.row.amount).toFixed(2) || '0.00' }}
						</template>
					</el-table-column>
					...
					<!-- <el-table-column prop="orderNo" label="支入订单数">
					</el-table-column>
					<el-table-column prop="recordNo" label="支出订单数">
					</el-table-column> -->
	...
	export default {
		...
		methods: {
			...
			getLoadData(pageNum, pageSize) {
				this.isLoad = true;
				$http({
					url: common.apihost + 'statisticsDto/checkAccount',
					data: {
						data: {
							page: pageNum,
							pagesize: pageSize,
			...
			download() {
				...
				window.location.href = common.apihost + 'statisticsDto/checkAccountPoi?code=' + this.formInline.channelId + '&startTime=' + this.formInline.startTime + '&endTime=' + this.formInline.endTime
				/*if(!timestamp2) {
					window.location.href = common.apihost + 'statisticsDto/checkAccountPoi?orderNo=' + this.formInline.orderNo + '&recordNo=' + this.formInline.recordNo + '&startOrderTime=' + this.formInline.startOrderTime + '&endOrderTime=' + this.formInline.endOrderTime + '&systemCode=' + this.formInline.systemCode + '&merchantId=' + this.formInline.merchantId + '&status=' + this.formInline.status;
				} else {
